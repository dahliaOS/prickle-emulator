...
extern TraceEvent _TRACE_KVM_HANDLE_DRC_READ_EVENT;
...
extern uint16_t _TRACE_KVM_HANDLE_DRC_READ_DSTATE;
...
#define TRACE_KVM_HANDLE_DRC_READ_ENABLED 1
...
#define TRACE_KVM_HANDLE_DRC_READ_BACKEND_DSTATE() ( \
    trace_event_get_state_dynamic_by_id(TRACE_KVM_HANDLE_DRC_READ) || \
    false)

static inline void _nocheck__trace_kvm_handle_drc_read(void)
{
    if (trace_event_get_state(TRACE_KVM_HANDLE_DRC_READ) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:kvm_handle_drc_read " "handle dcr read" "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 );
    }
...
static inline void trace_kvm_handle_drc_read(void)
{
    if (true) {
        _nocheck__trace_kvm_handle_drc_read();
    }
}
